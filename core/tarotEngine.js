...
class TarotEngine {
  static imageBaseURL() {
    return 'https://www.sacred-texts.com/tarot/pkt/img/';
  }

  static majorImageURL(id) {
    return `${this.imageBaseURL()}ar${String(id).padStart(2, '0')}.jpg`;
  }

  static minorImageURL(suit, number) {
    ...
    return `${this.imageBaseURL()}${suitCodes[suit]}${rank}.jpg`;
  }
  ...
  static generateMinorArcanaCard(suit, number) {
    ...
    return {
      ...
      element,
      number,
      imageURL: this.minorImageURL(suit, number),
  ...
  static generateFullDeck() {
    const deck = MAJOR_ARCANA.map(card => ({
      ...card,
      imageURL: this.majorImageURL(card.id)
    }));
